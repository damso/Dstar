	...
	for (CEntry_t::iterator it = m_array.begin(); it != m_array.end(); ++it) {
		...
		if (entry->getBand().Len() > 1U) {
			if (entry->getFrequency() != 0.0)
				desc.Printf(wxT("Data (D-Star)"), entry->getFrequency());
			else
				desc = wxT("Data (D-Star)");
		} else {
                        if (entry->getFrequency() != 0.0)
				desc.Printf(wxT("Voice (D-Star)"),
                                                entry->getFrequency(),
		...
		wxString output;
		output.Printf(wxT("%s-S>APDG01,TCPIP*,qAC,%s-GS:;%-7s%-2s*%02d%02d%02dz%s%cD%s%c&RNG%04.0lf/A=%06.0lf %s %s\r\n%s-S>APDG01:>Hotspot\r\n"),
...
void CAPRSWriter::sendIdFramesMobile()
{
	...
	for (CEntry_t::iterator it = m_array.begin(); it != m_array.end(); ++it) {
		...
		wxString output1;
		output1.Printf(wxT("%s-S>APDG01,TCPIP*,qAC,%s-GS:;%-7s%-2s*%02d%02d%02dz%s%cD%s%c&/A=%06.0lf"),
			m_gateway.c_str(), m_gateway.c_str(), entry->getCallsign().c_str(), entry->getBand().c_str(),
			tm->tm_mday, tm->tm_hour, tm->tm_min,
